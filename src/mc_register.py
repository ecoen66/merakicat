...
def Register(host_id,ios_username,ios_password,ios_port,ios_secret):
    ...
    ** Need to add show meraki compatibility check. **
    ...
    while x <= lines_to_test:
        ...
        res = [i for i in range(len(r_more[x+7])) if r_more[x+7].startswith(" - Incompatible", i)]
        if not len(res) == 0:
            ...
            ## We will loop for the number of time that "Incompatible" shows up
            ## on the switch line
            y = 0
            while y < len(res):
                offense = r_more[x+7]
    ...
    ## Find the first line number (top) of the final results table in the output
    regex = re.compile("^1")
    ...
    while z <= bottom:
        ...
        z += 1
    if debug:
        print("successfully", issues, registered_switches, registered_serials, nm_list)
