...
#            if intf_name == "GigabitEthernet1/1/1":
#                print(f"For {intf_name}, Switch_module = {Switch_module}.")
            ...
            try:
                ...
                if re.search(r'^GigabitEthernet',intf_name) and sub_module == "1":
...
#        parse = CiscoConfParse(config_file, syntax='ios', factory=True)
        ...
        obj = re.search(r'Ethernet(\d+)\/(\d+)\/(\d+)$',intf)
        if debug:
...
def Meraki_config(dashboard,sw_list,port_dict,Downlink_list,switch_name):
